...
from apps.scenario.api.serializers import ScenarioSerializer, ScenarioPreviewSerializer
from apps.core.permissions import IsAuthor, CanReadObject
from apps.scenario.api.views import (
    ScenarioPrivateFilteredByTag,
    ScenarioTagListView,
    ScenarioPublicFilteredByTag,
)
from apps.scenario.models import Scenario
from apps.core.views import TagDeleteView, TagCreateView

from ..models import Folder
from .serializers import UserSerializer, FolderSerializer, UserPOSTSerializer
...
class UserListContentView(generics.ListAPIView):
    ...
    def get_queryset(self):
        user = get_object_or_404(self.queryset, username=self.kwargs["username"])
        if user:
            ...
            if perm:
                ...
                if content_type == "scenario":
                    ...
                    if folder == child_folder:
                        return Response(
                            {
                                "non_field_erros": "You can't put a folder " \
                                "inside itself"
                            },
                            status=400,
                        )
                    ...
                    return Response(
                        {
                            "non_field_errors": f"{content_type} is not a valid content_type",
                        },
                        status=400,
                    )
